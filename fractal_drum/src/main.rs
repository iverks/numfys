...
fn main() {
    ...
    let griddy = Grid::from_fractal_marked_double(&fractal, side_length, level);
    ...
    // griddy.solve_sparse();
    let (eigs, sol) = griddy.solve_sparse();

    plot_sln(sol.slice(s![0, .., ..]), "images/sparsesolution0.jpg").unwrap();
    plot_sln(sol.slice(s![1, .., ..]), "images/sparsesolution1.jpg").unwrap();
    plot_sln(sol.slice(s![2, .., ..]), "images/sparsesolution2.jpg").unwrap();
